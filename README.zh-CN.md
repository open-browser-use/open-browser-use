...
在底层，你的智能体调用同一个 `js` 工具，并面向一套 Playwright 风格的 SDK（即 `agent` 全局对象）来编写代码。一整轮浏览器操作大致就这么短小：
...
| 能力                         | 含义                                                                                                             |
| ---------------------------- | --------------------------------------------------------------------------------------------------------------- |
| **操作元素**          | 点击、填写、输入、按键、选择、悬停 —— 以角色、文本或 CSS 来定位（即更稳健的 Playwright 方式）。                  |
| **凭视觉或 DOM id 点击** | 当没有干净的选择器时，可采用基于视觉/坐标和基于 DOM 寻址的方式 —— 包括跨越跨域 iframe。 |
| **读取与提取**           | 文本、表格、属性以及截图。                                                                                  |
| **文件与对话框**          | 上传、下载、alert 和 confirm。                                                                                  |
| **标签页、会话与续作**  | 同时驾驭多个标签页和会话，并能跨多轮续作长任务而不丢失进度。                             |

## 工作原理

你的智能体把 open-browser-use 当作一个 MCP 服务器来对话。它通过单一的 `js` 工具编写 JavaScript，这些代码在一个常驻的 Node 运行时中执行，其中 `agent` 就是那套 SDK。这些调用以 JSON-RPC 的形式，经由一个受能力门控、仅属主可访问的 Unix socket 传送给 **`obu-host`** —— 一个按会话隔离的代理（broker），它通过以下两种后端之一来驱动你的浏览器：
...
- **WebExtension 后端** —— 通过 open-browser-use 扩展驱动一个正常安装的 Chrome。无需 `--remote-debugging-port`，你真实的配置文件和登录态都完好无损。这是日常使用的默认方式。
- **CDP 后端** —— 接入任何以远程调试方式启动的 Chrome（`OBU_CDP_URL`）。非常适合无头（headless）和脚本化运行。

> [!IMPORTANT]
> 一切都留在你的机器上。`obu-host` 的 socket 仅属主可访问，并按操作系统用户进行身份验证；只有受信任的 SDK 代码才持有触达它所需的能力令牌 —— open-browser-use 绝不会向任何远程服务发起调用。
...
- **高层辅助方法**（`tab.act.*`、`tab.flows`、`tab.read`），均构建于同一套原语之上。
...
open-browser-use 绝不会调用任何远程 URL 或产品策略服务。SDK guard 与 host policy 都在本地运行，且默认采用宽松策略。当你需要时，可通过环境变量来收紧它们：
...
SDK 调用方还可以为每个浏览器安装针对导航、下载、上传、历史记录和原始 CDP 的 `Guards` 钩子 —— 它们在你本地的智能体进程中运行，不会发起任何网络请求：
